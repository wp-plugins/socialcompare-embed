...
function socialcompare_admin_menu(){
	if (function_exists('add_submenu_page')) {
		...
		add_submenu_page('plugins.php', __('SocialCompare embed options','socialcompare'), 'SocialCompare', 'manage_options', 'socialcompare-plugin-page', 'socialcompare_plugin_page');
	}
...
function socialcompare_update_size($param_name){
	if (!empty($_POST[$param_name])){
		...
		if (preg_match('#^\d+(%|px)?$#', $size)) {
			...
			return __('Size saved', 'socialcompare').'<br />';
		}
		return '<strong style="color:red">'.__('Invalid size value', 'socialcompare').'</strong><br />';
	}
	return false;
}
...
function socialcompare_plugin_page(){
	/*
	delete_option('socialcompare_width');
	delete_option('socialcompare_height');
	*/
	...
	if (count($_POST)>0) {
		...
		if (!empty($_POST['socialcompare_design'])){
			...
			if (preg_match('/^#?((\s*;\s*)?([a-z\\-])+(:[0-9a-zA-Z#\\.\\-\\s,]+)?)+$/m', $socialcompare_design)) {
				...
				$result.=__('Design option saved', 'socialcompare').'<br />';
			}
			else {
				$result.='<strong style="color:red">'.__('Design option invalid', 'socialcompare').'</strong><br />';
			}
		}
	...
	//TODO hide languages option
	//TODO hide fullscreen option
	//TODO default css/design option
	?><h2><?php _e('SocialCompare Configuration', 'socialcompare'); ?></h2>
<p><?php _e('You can specify here the default size and design options to use when embedding a SocialCompare comparison table using [socialcompare] or [sc] shortcode.', 'socialcompare')?></p>
<form action="" method="post">
<p><label for="socialcompare_width"><?php _e('Default width', 'socialcompare'); ?></label> <input name="socialcompare_width" type="text" size="15" maxlength="12" value="<?php echo $socialcompare_width; ?>" /> ('100%', '500')</p>
<p><label for="socialcompare_height"><?php _e('Default height', 'socialcompare'); ?></label> <input name="socialcompare_height" type="text" size="15" maxlength="12" value="<?php echo $socialcompare_height; ?>" />  ('450', '600')</p>
<p><label for="socialcompare_design"><?php _e('Default design options', 'socialcompare'); ?></label> <small>(<?php _e('More details:', 'socialcompare'); ?>
...
<h2><?php _e('How to embed SocialCompare\'s comparison?', 'socialcompare'); ?></h2>
<p><?php _e('Once you have a comparison URL you can use following syntax in your blog post or page.', 'socialcompare')?></p>
...
<p><?php _e('You can override the default dimensions specified above using this syntax.', 'socialcompare')?></p>
...
function socialcompare_register_plugin_links($links, $file) {
	...
	if ($file==$base){
		...
		//XXX already exists $links[]='<a href="http://socialcompare.com/">SocialCompare.com</a>';
	}
	return $links;
}
